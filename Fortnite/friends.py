...
async def event_friend_add(self, Friend):
    ...
    if self.Settings["InviteFriendOnFriendAdded"] or Friend.id in self.Settings["GiveFullAccessTo"]:
        ...
        print(colored.Colored(f'[BOT {self.user.display_name}] [{TimeInUTC}] {Friend.display_name} is now your friend', "green"))
...
async def event_friend_remove(self, Friend):
    if self.Settings["SendFriendRequestOnFriendRemove"] or Friend.id in self.Settings["GiveFullAccessTo"]:
        try:
            ...
            print(colored.Colored(f'[BOT {self.user.display_name}] [{TimeInUTC}] {Friend.display_name} removed you as a friend, bot sent him a friend request', "green"))
        except fortnitepy.errors.HTTPException as Error:
            print(colored.Colored(f'[BOT {self.user.display_name}] [{TimeInUTC}] {Friend.display_name} removed you as a friend', "red"))
    else:
        print(colored.Colored(f'[BOT {self.user.display_name}] [{TimeInUTC}] {Friend.display_name} removed you as a friend', "red"))
