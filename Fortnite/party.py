...
async def event_party_member_join(self, Member):
    if Member.id == self.user.id:
        ...
        if self.user.party.me.is_leader and self.user.party.member_count == 1:
            print(colored.Colored(f'[BOT {self.user.display_name}] [{TimeInUTC}] Created Party', "green"))
        else:
            print(colored.Colored(f'[BOT {self.user.display_name}] [{TimeInUTC}] Joined Party', "green"))
        ...
        if CID.status != 200:
            print(colored.Colored(f'Can\'t find {self.Settings["DefaultSkin"]}',"red"))
        ...
        if self.Settings["EmoteAfterJoiningParty"] and not self.Settings["EmoteName"] == "":
            ...
            if EID.status != 200:
                print(colored.Colored(f'Can\'t find {self.Settings["EmoteName"]}',"red"))
            else:
        ...
        if self.Settings["DefaultBackpack"] != "":
            ...
            if BID != 200:
                print(colored.Colored(f'Can\'t find {self.Settings["DefaultBackpack"]}',"red"))
            else:
                await self.user.party.me.set_backpack(asset=BID.id)
    else:
        print(colored.Colored(f'[BOT {self.user.display_name}] [{TimeInUTC}] {Member.display_name} Joined the Party', "green"))
